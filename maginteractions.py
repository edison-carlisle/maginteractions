...
class MagInteractions:
    ...
    def __init__(self, magstruc = None, temperatures = [1.0], spin_squared = 1.0,
                 j_ij = [1.0], r_max = 10.0, spin_dim = 3, lat_vecs = None,
        ...
        r_nn_arg = np.argmin(np.apply_along_axis(np.linalg.norm, 1, self.magstruc.atoms[1:]
                                                 - self.magstruc.atoms[0]))
        self.r_nn_vec = self.magstruc.atoms[r_nn_arg + 1] - self.magstruc.atoms[0]
        self.r_nn = np.linalg.norm(self.r_nn_vec)
        self.g_nn = np.sqrt(self.magstruc.gfactors[r_nn_arg + 1] * self.magstruc.gfactors[0])
    ...
    def filter_spin_positions(self, r_max = None):
        ...
        for uu in calc_idxs:
            calc_idxs_new.add(np.where(np.array(list(indices_to_keep)) == uu)[0][0])
    

        self.magstruc.atoms = self.magstruc.atoms[list(indices_to_keep)]
        self.magstruc.spins = self.magstruc.spins[list(indices_to_keep)]

        self.magstruc.calcIdxs = list(calc_idxs_new)
    ...
    def calculate_orf_real_space(self, lmop = 1.0, corr_length = 0.0, damping_matrix = None,
        ...
        for uu in calc_idxs:
            ...
            if correlation_method == 'full':
                ...
                sj = np.where(sj == np.inf, s_xyz, sj)
            else:
                sj = s_xyz

            d_xyz = rj - ri
            d2_xyz = np.sum((d_xyz) ** 2, axis = 1).reshape(d_xyz.shape[0], 1)
            d1_xyz = np.sqrt(d2_xyz)
            d1_xyzr = d1_xyz.ravel()

            sisj = np.sum(si * sj, axis = 1)
            s1 += np.histogram(d1_xyzr, bins = r_bin, weights = sisj)[0]
    ...
    def calculate_orf_reciprocal_space(self, n_bz_vecs = 2, n_recip_lats = 2, 
        ...
        for h in range(-n_recip_lats, n_recip_lats + 1):
            for k in range(-n_recip_lats, n_recip_lats + 1):
                for l in range(-n_recip_lats, n_recip_lats + 1):
                    pt = np.array([h, k, l]) @ self.recip_lats
    ...
    def calculate_damping_matrix(self, ord_vec = None, mode = -1, isotropic = False,
        ...
        if j_0 is None:
            if ord_vec is None:
                raise ValueError('If j_0 is not provided, ord_vec must be provided to' \
                'calculate j_0.')
            j_0 = self.calc_j_q(ord_vec)[mode]

        if j_hessian is None:
            if ord_vec is None:
                raise ValueError('If j_hessian is not provided, ord_vec must be provided to ' \
                'calculate j_hessian.')
            j_hessian = self.calculate_j_hessian(ord_vec, mode = mode)
        ...
        if isotropic:
            damping_amplitude = np.linalg.det(damping_matrices)**(1 / np.shape(damping_matrices)[1])
    ...
    def calc_3d_damping_matrix(self, damping_matrix = None, subspace_vecs = None):
        ...
        if subspace_vecs is None:
            raise ValueError('Subspace eigenvectors must be provided to calculate full 3D damping matrix.')

        if len(damping_matrix.shape) == 3:
            if damping_matrix.shape[1] != subspace_vecs.shape[1]:
                raise ValueError('Damping matrix dimensionality does not match number of subspace eigenvectors.')
        elif damping_matrix.shape[0] != subspace_vecs.shape[1]:
            raise ValueError('Damping matrix dimensionality does not match number of subspace eigenvectors.')

        damping_matrix_3d = subspace_vecs @ damping_matrix @ subspace_vecs.T
        # damping_matrix_3d += (2.0 / np.min(damping_matrix_3d)
        #                       * (np.identity(3) - subspace_vecs @ subspace_vecs.T))

        # background_damping = (1e2 * np.max(np.linalg.eigvals(damping_matrix_3d), axis = 1)
        #                       * (np.identity(3) - subspace_vecs @ subspace_vecs.T))

        background_damping_amplitude = np.max(np.linalg.eigvals(damping_matrix_3d), axis = 1)
        background_damping_amplitude *= 1e4
        background_damping_matrix = (np.identity(3) - subspace_vecs @ subspace_vecs.T)

        if type(background_damping_amplitude) == float:
            background_damping_amplitude = np.array([background_damping_amplitude])

        background_damping =  np.einsum('i,jk->ijk', background_damping_amplitude,
                                        background_damping_matrix)

        damping_matrix_3d = (damping_matrix_3d + background_damping)        
        damping_matrix_3d = damping_matrix_3d.real
        
        # the extra term added to the damping matrix is to ensure that the
        # correlations outside of the subspace are sufficiently damped so that they 
        # don't contribute to the ORF, which is only calculated from the correlations
        # within the subspace. The factor of 1e4 times the maximum eigenvalue of the
        # the original damping matrix is chosen somewhat arbitrarily to ensure that
        # the damping is strong enough without causing numerical issues.
    ...
    def calculate_lmop(self, n_ord_vec = 1, dim = 3, damping_matrices = None,
        ...
        if j_hessian.shape[0] != dim:
            raise ValueError('Hessian dimensionality does not match specified' \
            'correlation dimensionality.')
        
        if dim != 3 and subspace_vecs is None:
            raise ValueError('Subspace eigenvectors must be provided for 1D and 2D' \
        ...
        if damping_matrices is None:
            if not use_stored_damping_matrices or self.damping_matrices is None:
                if j_0 is None:
                    if ord_vec is None:
                        raise ValueError('If j_0 is not provided, ord_vec must be' \
                        'provided to calculate j_0.')
                    j_0 = self.calc_j_q(ord_vec)[mode]

                damping_matrices = self.calculate_damping_matrix(j_0 = j_0, j_hessian = j_hessian,
                                                                 temperatures = temperatures,
                                                                 orfs = orfs, store = store)
            else:
                damping_matrices = self.damping_matrices

        if dim == 1:
            r_nn = self.r_nn_vec @ subspace_vecs # make r_nn 1D
            chi_0 = self.calc_chi_0(temperatures = temperatures)

            c_0 = 1/chi_0 - j_0 + orfs
            decay_term = np.exp(-r_nn * np.sqrt(damping_matrices))
            decay_term = decay_term.ravel() # make decay term 1D
        ...
        if dim == 2:
            r_nn = self.r_nn_vec @ subspace_vecs # make r_nn 2D
    ...
    def calculate_correlation_parameters(self, ord_vec, n_ord_vec = 1, mode = -1, hstep = 1e-4,
            ...
            if sum_rule_orf:
                ...
                for i in range(self.n_temps):
                    ...
                    def orf_residual(orf):
                        ...
                        if dim != 3:
                            damping_matrix = self.calc_3d_damping_matrix(damping_matrix = damping_matrix,
                                                                         subspace_vecs = subspace_vecs) 
        ...
        if dim != 3:
            self.damping_matrices = self.calc_3d_damping_matrix(damping_matrix = self.damping_matrices,
                                                                subspace_vecs = subspace_vecs)